...
function TDSequential(ohlc) {
  ...
  ohlc.forEach((item, i) => {
    ...
    if (i >= 5) {
      ...
      resultObj.sellSetup = false;//result[i - 1].sellSetup;
      resultObj.buySetup = false;//result[i - 1].buySetup;
      resultObj.TDSTBuy = result[i - 1].TDSTBuy;
      resultObj.TDSTSell = result[i - 1].TDSTSell;
      resultObj.sellSetupPerfection = false;//result[i - 1].sellSetupPerfection;
      resultObj.buySetupPerfection = false;//result[i - 1].buySetupPerfection;
