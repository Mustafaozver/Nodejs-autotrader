	...
	*/
	...
	LoadPage:function(){
		...
		try{
			...
			if(xcode)setTimeout(xcode, 2000);
			else throw new Error("saved page is not existed.");
		console.log("DBManager isn",xcode);
		}catch(e){
...
var IdealWindow = function(title){
	this.JQueryObject = $("#idealwindow").clone(true);
	this.OnClose = function(){};
	var THAT = this;
	$(this.JQueryObject).kendoWindow({
		actions: [
			//"Custom",
			"Minimize",
			"Maximize",
...
IdealWindow.prototype.GetContent = function(){
	 return $(this.JQueryObject);//.children("div#idealwindow");
};
IdealWindow.prototype.SetTitle = function(title){
	$(this.JQueryObject).data("kendoWindow").title(title);
};
IdealWindow.prototype.Open = function(){
	$(this.JQueryObject).data("kendoWindow").open();
	//$(this.JQueryObject).data("kendoWindow").center();
	$(this.JQueryObject).data("kendoWindow").wrapper.find(".k-i-custom").parent().click(function (e) {
		console.log("Custom action button clicked");
		e.preventDefault();
	});
};
IdealWindow.prototype.GetPosition = function(){
	var win = $(this.JQueryObject).data("kendoWindow");
	var position = win.wrapper.offset();
	return [Math.floor(position.left), Math.floor(position.top)];
};
IdealWindow.prototype.SetPosition = function(x,y,w,h){
	$(this.JQueryObject).data("kendoWindow").setOptions({
		position:{
			left:x,
			top:y,
		},
...
IdealWindow.prototype.GetSize = function(){
	var win = $(this.JQueryObject).data("kendoWindow");
	var width = Math.floor(win.wrapper.width())+3;
	var height = Math.floor(win.wrapper.height())+3;
	return [width, height];
};
...
DepthWindow.Config = {
	...
	TradeGrid:{ // trade table
		...
		columnDefs:[
			...
			{field:"Seller", headerName:"Satan", headerClass:"Derinlik_Satanlar"},
			{field:"Buyer", headerName:"Alan", headerClass:"Derinlik_Alanlar"},
...
ProfitageWindow.Config = {
	...
	ProfitageGrid:{
		...
		columnDefs:[
			...
			{field:"High", headerName:"Yüksek", headerClass:"Derinlik_Satanlar",},
			{field:"Low", headerName:"Düşük", headerClass:"Derinlik_Alanlar",},
		],
		defaultColDef:{
			minWidth: 50,
			editable:false,
...
ATA.Loops.push(function(){
	...
	if(PivotTime < lastPivotTime){
		for(var key in TVChartWindow.Windows){
			//IdealInterFace.Profitage(TVChartWindow.Windows[key].Symbol);
		}
	}
...
PriceTableWindow.prototype.UpdateList = function(){
	...
	var CalculateColor = function(num,rising){
		...
		var R = Math.pow(1 - num, 0.7) * 0.703 + 0.156;
		var G = Math.pow(num,0.7) * 0.445 + 0.207;
		var B = (num - 0.5)/5 + 0.15;
		
		R = "" + Math.floor(255*R).toString(16);if(R.length < 2) R = "0" + R;
		G = "" + Math.floor(255*G).toString(16);if(G.length < 2) G = "0" + G;
		B = "" + Math.floor(255*B).toString(16);if(B.length < 2) B = "0" + B;
	...
	arr.map(function(item){
		...
		obj.innerHTML = "<H5>" + item + "</H5>% " + deg.toFixed(2);
		div.append(obj);
	});
};
...
ATA.Loops.push(function(){
	...
	if(PivotTime < lastPivotTime){
		for(var key in PriceTableWindow.Windows){
			//IdealInterFace.Profitage(PriceTableWindow.Windows[key].Symbol);
		}
	}
...
SplitageChartWindow.Config = {
	...
	SplitageTable:{
		...
		columnDefs:[
			...
			{field:"Cost", headerName:"Maaliyet"},
		],
		defaultColDef:{
			minWidth: 50,
			editable:false,
...
IdealWindow.Windows = {};
IdealWindow.ActiveWindowID = false;
IdealWindow.Action = false;
IdealWindow.Container = false;
IdealWindow.CheckSnep = function(win){
	...
	for(var i=0;i<PinnablePoints.length;i++){
		...
		if(Math.abs(point[0] - newX) < diffpixel){
			mnewX = point[0];
			break;
		}else if(Math.abs(point[2] - newX) < diffpixel){
			mnewX = point[2];
			break;
		}
	}
	...
	for(var i=0;i<PinnablePoints.length;i++){
		var point = PinnablePoints[i]; // x1, y1, x2, y2
		if(point[4] == win.ID)continue;
		if(Math.abs(point[1] - newY) < diffpixel){
			mnewY = point[1];
			break;
		}else if(Math.abs(point[3] - newY) < diffpixel){
			mnewY = point[3];
			break;
		}
	}
...
IdealWindow.prototype.Initialize = function(){
	...
	$(this.Div).find(".Title").mousedown(function(event){
		...
		IdealWindow.ActiveWindowID = THAT.ID;
	...
	$(this.Div).find(".Restore").click(function(){
		THAT.Restore();
	});
	$(this.Div).find(".Maximize").click(function(){
		THAT.Maximize();
	});
	$(this.Div).find(".Minimize").click(function(){
		THAT.Minimize();
	});
};
...
IdealWindow.Setup = function(){
	...
	$(document.body).mousemove(function(event){
		...
		//THAT.
		if(IdealWindow.ActiveWindowID)switch((""+IdealWindow.Action).toUpperCase()){
			default:
			break;
			case "MOVE":
				var win = IdealWindow.Windows[IdealWindow.ActiveWindowID];
				var pincheck = IdealWindow.CheckSnep(win);
				var newX = event.pageX - win.__R.__X;
				var newY = event.pageY - win.__R.__Y;
				var mnewX = newX;
				var mnewY = newY;
